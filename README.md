...
Nexus 是一种“陪伴式桌面 AI”。当前公开稳定版是 **v0.4.7**：

- **桌宠形象**：伙伴以 Live2D 或分层立绘 v4 常驻桌面，而不是只存在于聊天记录里。导入 Cubism 模型会在激活前检查资源；缺动作或表情仍可显示并说明限制。
- **自然对话**：文字是默认入口；语音输入/输出可选，目录里包含 Grok STT/TTS。
- **长期记忆**：本机记住偏好、事件和关系线索；夜间整理会检测矛盾并降权，不把原始桌面内容送进模型。
- **本地优先，可接 API**：默认可以走 Ollama；也可以使用 DeepSeek、Qwen 3.8-max、GLM-5.3 等云端目录。密钥留在本机。
- **授权辅助**：搜索、天气、提醒等只在你明确授权后工作，并且可确认、可停止、可审计。
...
> 这一节是项目起点的设计约束，不是当前稳定版的功能上限。当前公开产品是 v0.4.7。
...
v0.4.7 是当前公开稳定版。第一阶段最小闭环已经成立，默认体验建立在它上面：

- 当前稳定主题是形象兼容、分层立绘 v4 和模型目录：导入 Live2D 前检查 Moc、纹理和已声明资源；分层立绘包是默认图片路径；Grok 语音与 Qwen 3.8-max / GLM-5.3 进入默认目录。从单张图生成 sprite atlas 仍不在稳定版里。
- 桌面陪伴感知保持短期、粗粒度、可暂停和可清理；时间说法是“一会儿 / 半小时左右”，不是精确到分秒。
...
当前最有价值的贡献不是把下一阶段提前做完，而是把已经公开的陪伴体验做扎实：

- 让桌宠窗口更稳定、更轻量。
- 让 Live2D / 立绘 v4 导入和切换更清楚，但不过度复杂。
- 让模型配置、语音和错误提示更好懂。
- 让文字对话、记忆可见性和暂停路径稳定可靠。
...
0.4 当前稳定版说明见
[RELEASE-NOTES-v0.4.7.md](docs/RELEASE-NOTES-v0.4.7.md)，
发布加固清单见
...
Nexus 的长期想象空间很大，但当前最重要的是把已经公开的产品和下一阶段分开：

**Nexus 是一个住在电脑里的 AI 伙伴。**

v0.4.7 已经能常驻桌面、对话、记住重要的事，并在授权后提供轻量帮助。0.5 才做桌宠跟随鼠标、打字反应和窗口互动；签名安装包也还不是当前发行策略。
