...
<p align="center">
  <b>v0.1.1</b> · Windows 桌面 AI 陪伴体 · Desktop AI Companion
</p>
...
Nexus 是一个面向 Windows 的桌面 AI 陪伴应用，集成 Live2D 角色渲染、连续语音对话、长期记忆、桌面感知、自主行为与多平台集成能力。

### 核心功能

- **桌宠 + 面板** 双视图，Live2D 角色渲染与表情/动作/情绪联动
- **连续语音对话** — 多引擎 STT/TTS，支持唤醒词、VAD 语音活动检测、连续对话、语音打断
...
| 语音输入 | Sherpa-onnx · SenseVoice · Paraformer · GLM-ASR-Nano · 智谱 ASR · 火山引擎 · 腾讯 ASR · ElevenLabs |
| 语音输出 | Edge TTS · MiniMax · 火山引擎 · CosyVoice · OmniVoice · DashScope Qwen3-TTS · OpenAI TTS · ElevenLabs |
| 对话模型 | OpenAI · Anthropic · Gemini · DeepSeek · Kimi · Qwen · GLM · Grok · Ollama 等 18+ 提供商 |
...
Nexus is a Windows desktop AI companion featuring Live2D character rendering, continuous voice conversation, long-term memory, desktop awareness, autonomous behavior, and multi-platform integrations.

### Features

- **Pet + Panel** dual-view with Live2D character expressions, motion, and mood
- **Voice interaction** — multi-engine STT & TTS with wake word, VAD, continuous conversation, and speech interruption
...
- **Tool calling** — web search (with auto content extraction), weather, reminders, MCP protocol
