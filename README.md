...
- **连续语音对话** — 多引擎 STT（Sherpa、SenseVoice、FunASR、腾讯 ASR、浏览器识别）与 TTS（Edge TTS、MiniMax、火山引擎、OmniVoice、本地 Sherpa TTS），支持唤醒词、VAD 语音活动检测、连续对话、语音打断
...
| 语音输出 | Edge TTS · MiniMax · 火山引擎 · OmniVoice · Sherpa TTS · 系统语音 |
...
- **連續語音對話** — 多引擎 STT（Sherpa、SenseVoice、FunASR、騰訊 ASR、瀏覽器識別）與 TTS（Edge TTS、MiniMax、火山引擎、OmniVoice、本地 Sherpa TTS），支援喚醒詞、VAD、連續對話、語音打斷
...
- **Voice interaction** — multi-engine STT (Sherpa, SenseVoice, FunASR, Tencent ASR, Web Speech API) & TTS (Edge TTS, MiniMax, Volcengine, OmniVoice, local Sherpa TTS); wake word, VAD, continuous conversation, speech interruption
...
- **音声対話** — マルチエンジン STT（Sherpa・SenseVoice・FunASR・テンセント ASR・Web Speech API）& TTS（Edge TTS・MiniMax・火山エンジン・OmniVoice・ローカル Sherpa TTS）、ウェイクワード・VAD・連続会話・音声割り込み対応
...
- **음성 대화** — 멀티엔진 STT(Sherpa·SenseVoice·FunASR·텐센트 ASR·Web Speech API) & TTS(Edge TTS·MiniMax·화산엔진·OmniVoice·로컬 Sherpa TTS), 웨이크워드·VAD·연속 대화·음성 인터럽트
