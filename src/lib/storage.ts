...
const defaultSettings: AppSettings = {
  ...
  speechInputApiBaseUrl: 'https://openspeech.bytedance.com/api/v3/auc/bigmodel',
  speechInputApiKey: '',
  speechInputModel: 'bigmodel',
...
export function loadSettings(): AppSettings {
  ...
  const migrateLegacySpeechInputProvider = (
    isBrowserSpeechInputProvider(requestedSpeechInputProviderId)
    || requestedSpeechInputProviderId === 'local-sensevoice'
    || requestedSpeechInputProviderId === 'local-paraformer'
  )
  const effectiveSpeechInputProviderId = migrateLegacySpeechInputProvider
    ? defaultSettings.speechInputProviderId
    : requestedSpeechInputProviderId
  const hasLocalSpeechInputProvider = false
